...
from python_docker_operator.interface import VariableInterface
...
class SpotifyApi:
    ...
    def connect(self):
        ...
        self._client = spotipy.Spotify(
            auth_manager=SpotifyOAuth(self._client_id, self._client_secret, self.REDIRECT_URI, scope=scope, cache_path=self.CACHE)
        )
...
if __name__ == '__main__':
    ...
    CLIENT_ID = VariableInterface('SPOTIFY__TRACKS_API_CLIENT').env_variable
    CLIENT_SECRET = VariableInterface('SPOTIFY__TRACKS_API_SECRET').env_variable
    assert CLIENT_ID and CLIENT_SECRET

    client = SpotifyApi(CLIENT_ID, CLIENT_SECRET)
    client.connect()
    client.put_new_tracks_in_playlist_from_followed_artists(lookback_days=7)
